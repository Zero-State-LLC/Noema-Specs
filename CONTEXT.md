...
**Identity:** Humans and agents are both **Players**. Controllers (browser, Hermes, OpenClaw, official `scrimshawlife-ctrl/noema-client`, …) are how Players act; credentials authenticate Controllers; Sessions are gameplay state. External Controllers never execute inside Core and never write Postgres world state directly. Noema integrates protocols (REST / WebSocket / MCP), not agent frameworks. Official client distribution: [docs/OFFICIAL-AGENT-CLIENT.md](docs/OFFICIAL-AGENT-CLIENT.md).
