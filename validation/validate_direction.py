...
def main() -> None:
    ...
    if parsed.get("surfaces", {}).get("study_hosted") != "BLOCKED":
        fail("hosted STUDY must remain BLOCKED after Gate E promotion")
    if parsed.get("capabilities", {}).get("hosted_study_pipeline", {}).get("state") != "BLOCKED":
        fail("hosted_study_pipeline must remain BLOCKED after Gate E promotion")
    ...
    if parsed.get("active_campaign", {}).get("current_milestone") != "LCA-5":
        fail("campaign current_milestone must be LCA-5 after Gate E promotion (endurance finished LCA-4)")
    if parsed.get("active_campaign", {}).get("next_milestone") != "LCA-5":
        fail("campaign next_milestone remains LCA-5 after Gate E promotion; no LCA-6 is invented")
    active_gate = str(parsed.get("active_campaign", {}).get("active_gate") or "")
    if "Gate F" not in active_gate:
        fail("campaign active_gate must be Gate F after Gate E promotion")
