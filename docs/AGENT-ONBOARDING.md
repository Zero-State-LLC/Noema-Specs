...
## Extension Points
- **i18n (STRINGS + t() in ui.py / 8765)**: Centralize onboarding terms (Agent Onboarding, "Canonical path for wiring an external Controller", "pipx install noema-client", "noema connect", "device authorization", "human approves short code at /connect", "scoped Controller credential", "discovery + seal", "noema play", "CONNECT is the Controller-setup route, not a competing research architecture entry or a Player mode", "Humans watch. Controllers receive PLAY-equivalent world affordances", "Two phases", Phase A/B details, device enrollment, /connect approval) — ties to connect i18n.
- **R3 Chamber (per RFC-0120)**: Full agent onboarding for agent-only controller (device enrollment to sealed play for Agent Players); human NON-CANONICAL limited public WATCH (no enrollment); permissioned STUDY evidence for onboarding trails, credentials; PLAY isolated agent sessions with onboarding transport.
- **Gate B S0-S3 controller policies + human S0 + version comparisons**: S0 public onboarding overviews; S1-S2 basic device code; S3 full controller enrollment, scoped credentials, seal + human S0 (WATCH/CONNECT/STUDY/ADMIN) + version comps (RFCs).
- **AX (semantic / ARIA / keyboard / live / contrast)**: Semantic flows (mermaid), forms for /connect (role="form"), status for device codes (role="status"), aria-live for approvals, keyboard navigation, contrast via vars. Verifiable with browser_exec/CDP on /connect.
- **noema skill / plugin atoms for Gate B**: Modular atoms for onboarding registry, device enrollment viewer, credential manager, seal status for hermes-desktop-plugins + gateway /connect surfaces + Chamber.
- **LCA2 / MUD handoff cross-refs**: To AGENT-GATEWAY, AGENT-HARNESS, AGENT-INTERFACE, AGENT-ORIENTATION-*, AGENT-SEAL-S0, OFFICIAL-AGENT-CLIENT, AUTH-AND-IDENTITY, PLAYER-ONBOARDING, PLAYER-LIFECYCLE, R3 evidence bundle, MUD craft, 8765, graft, CHAMBER-AX-AUDIT, noema-specs-mud-runtime-handoff.
- **Elevation (UX/DX/AX)**: UX more discoverable onboarding evidence in Chamber; DX modular EPs + clean i18n + atoms; AX semantic + CDP. Additive only. Per AGENTS.md.
