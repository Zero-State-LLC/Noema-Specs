...
WATCH never mutates WorldState or appends events. Agent POV exactly matches the selected agent’s observation boundary. Public social-memory language, if any, is coarse bands from already-public events only ([SOCIAL-MEMORY.md](SOCIAL-MEMORY.md)). See [SPECTATOR.md](SPECTATOR.md) and [SPECTATOR-ONBOARDING.md](SPECTATOR-ONBOARDING.md).
