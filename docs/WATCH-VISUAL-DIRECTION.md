...
Non-normative implementation and review seams within the existing visual contract; this section neither implements the redesign nor reports a completed audit.

- **Seam:** refine deterministic map/event coupling, compact legend placement, and responsive density using the same authorized semantic graph in TEXT and PIXEL. Compare selection, follow, uncertainty, and public-residue treatments against the existing Phosphor glyph atlas rather than inventing a second renderer vocabulary or salience calculation.
- **Retained invariants:** `watch-live/1.0` and server-derived event tiers remain authoritative. Unknown topology stays absent, including in spacing and interaction hints; an unlocated event stays textual. TEXT remains complete, Canvas adds no unique facts, only one MAJOR treatment is active, idle scenes remain still, and reduced motion removes animation without removing meaning. Admin/research metrics and private cognition remain off public WATCH.
- **Compatibility/promotion:** composition changes can reuse the current endpoint and schema, but any new field, route, visibility grant, or event semantics requires separate change control. Preserve filtering, follow state, freshness/incident handling, keyboard access, and the single semantic layout during migration. A visual prototype or this Specified document is not hosted acceptance evidence; bind implementation observations to the deployed revision before claiming promotion.
- **Proposed checks:** compare TEXT/PIXEL facts and layout using identical public snapshots; include empty, stale, incident, dense, unlocated-event, and hidden-topology cases. Check mobile map primacy without horizontal scrolling, keyboard selection/follow and visible focus, screen-reader equivalence, contrast and non-color distinctions, and long translated labels without truncating meaning. Exercise reduced-motion and idle states, concurrent MAJOR events, unsafe labels/XSS, and unauthorized metrics; record observed results separately from unrun runtime, localization, or accessibility checks.
