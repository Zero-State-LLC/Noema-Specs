...
3. **Rehabilitation is already victim-specific and already pinned.** The reviewed input says decay and "three accepted trades" erase hostility "regardless of severity, victim, restitution." In fact `rehab_trades: 3` is pinned by [social-memory-catalog.gc3-s4.json](../specs/social-memory-catalog.gc3-s4.json) (schema `"const": 3`), and `rehabbedHostile()` counts trades **with the harmed counterparty only**, after the hostile cycle. It is not a runtime invention and not victim-blind. The criticism that survives is narrower still, and is registered as `B7e`: severity does not change the requirement. **Corrected 2026-08-31:** an earlier version of this line also claimed the pinned rule contradicted SOCIAL-MEMORY.md's "restitution trades". It does not. [RFC-0036](../rfcs/RFC-0036-decay-rehab.md) pins "3 distinct `TRADE_ACCEPTED`"; the "restitution" phrases in SOCIAL-MEMORY.md and GC3-S4 row F are illustrative prose in a high-level table, not a competing requirement. No authority pins a restitution trade type.
