...
## Extension Points
- **i18n (STRINGS + t() in ui.py / 8765)**: Centralize gateway terms (Agent Gateway, REST/WebSocket/MCP adapters, auth/sessions/capabilities/rate limits/protocol adapters, "Agent → direct database mutation", "authenticated request", "authorization", "action validation", "game engine", "canonical world-state mutation", threat mitigations, MVP boundary, terminology alignment "Player", "Controller + PlayerSession", "Credential with scopes", "Player principal (historical field name)"), connect/play/watch/study labels, "Connect · Agent Controller", "Approve Agent Connection", "External agents and local handles", etc. Ties to prior i18n sweeps (connect, study, watch, admin).
- **R3 Chamber (per RFC-0120)**: Full Agent Gateway responsibilities and edge enforcement in agent-only controller mode (full auth/sessions/capabilities/rate limits/protocol routing for Agent Players); human NON-CANONICAL limited public WATCH projections (observations only, no direct mutation); permissioned STUDY evidence for gateway logs, sessions, rate limits; PLAY isolated agent sessions with gateway transport.
- **Gate B S0-S3 controller policies + human S0 + version comparisons**: S0 public overviews of gateway architecture; S1-S2 basic adapters; S3 full controller enrollment, scoped credentials, rate limits, protocol adapters + human S0 (WATCH/CONNECT/STUDY/ADMIN only) + version comps (agent-protocol-v1, catalog).
- **AX (semantic / ARIA / keyboard / live / contrast)**: Semantic architecture diagrams, tables (role="table"), forms for auth (role="form"), aria-labels for adapters, aria-live for status (authorizing, ready), keyboard navigation, contrast via theme vars. Verifiable with browser_exec/CDP on /connect /play (tree, focus, announcements).
- **noema skill / plugin atoms for Gate B**: Modular atoms for gateway registry, session viewer, capability inspector, rate limit visualizer, protocol adapter status for hermes-desktop-plugins + gateway /connect surfaces + Chamber integration.
- **LCA2 / MUD handoff cross-refs**: To AGENT-INTERFACE, AGENT-HARNESS, AGENT-ONBOARDING, AGENT-ORIENTATION-*, AGENT-SEAL-S0, OFFICIAL-AGENT-CLIENT, AUTH-AND-IDENTITY, PLATFORM, PLAYER-ACTION-MAP, PLAYER-LIFECYCLE, DATA-MODEL, SECURITY, R3 evidence bundle, MUD craft, 8765/graft, CHAMBER-AX-AUDIT, noema-specs-mud-runtime-handoff.
- **Elevation (UX/DX/AX)**: UX more discoverable gateway evidence/architecture in Chamber; DX modular EPs + clean i18n + atoms + graft; AX semantic + CDP. Additive only. Per AGENTS.md.
