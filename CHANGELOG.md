...
- **Restore RFC-0116–RFC-0122 + RFC-0120 identity files** dropped by the Deep Time `#232` merge. Constitution (`rfcs/RFC-0120-agent-only-player-identity.md`, `docs/AGENT-ONLY-PLAYER-IDENTITY.md`, catalogs/attempts) and successor RFCs 0121/0122 are back. `CONTEXT.md`, README, AUTH, and related canon again say only agents are Players. HOSTED-FIRST-ENTRY / AUTH / AGENT-ONBOARDING follow RFC-0120. Device enrollment paths are `POST /v1/auth/device`. Hosted chrome is Home · Manifesto · Watch · Connect. ADR-006/007/008 files and pointers restored. SPEC-CHECKLIST Semantic Evolution and WATCH mapping boxes match shipped specs (`#231`, `#235`, Noema `#471`). Does **not** revert Deep Time / Semantic Evolution / WATCH mapping files. Remaining-work assimilates partner Specs `#236` (ENTER + MESSAGE), `noema-client#17`/`#18` (closed in 0.1.14), and Noema `#479`. Worker pin stays `5c796d6e` until an explicit deploy.
