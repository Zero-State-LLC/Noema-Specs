...
### Added
- **World Services Agent Contract + Discovery Rules.** Adds normative [WORLD-SERVICES-AGENT-CONTRACT.md](docs/WORLD-SERVICES-AGENT-CONTRACT.md) defining structured `service_id` + operations exposure for agents, observation shapes, AVAILABLE_ACTIONS representation, preconditions, and parity. Enhances [WORLD-SERVICES.md](docs/WORLD-SERVICES.md) with discovery rules (location vs institution bound), supersession doctrine (explicitly DEFERRED), observability, Genesis seeding, and per-service "Agent View" rows. Adds starter schema `specs/world-service-capability.schema.json`. Strengthens disambiguation from Player offices. Cross-references updated in AGENT-PLAY.md, PLAYER-ACTION-MAP.md, INSTITUTIONAL-AUTHORITY.md, FIRST-WORLD-SPEC-FREEZE.md. No new verbs, no authority change, no NPC services. See integration notes.
