...
- **Living Civilization Alpha Gate E endurance scenario companion (docs-only; NOT COMPLETE).** Adds `docs/LCA-GATE-E-SCENARIO.md` as the endurance evidence contract: Phase A four-hour → Phase B planned recovery → Phase C 24-hour, pins, honest incident/lag/stale marks, bounded interventions, and `PASS` / `FAIL` / `NOT_COMPUTABLE`. Suggested candidate `lca4-gate-e-endurance`. Gate D COMPLETE (#334 / `23987586`) is a prerequisite. Path 8 uses the existing Admin recover JSON; dedicated recovery-receipt objects remain **NOT_COMPUTABLE** (not invented). Links from acceptance, campaign LCA-4, Horizon 3 items 3–4, current-state comments, SPEC-GAP A5, and the same index surfaces that name the Gate D companion. Gate E remains unproven. Milestone stays LCA-4. No COMPLETE claim, Deploy, RFC-0130, hosted STUDY, Gate F `GO`, or runtime change.
