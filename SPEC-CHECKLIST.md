...
- [x] Admin Watch PIXEL scoped: Phosphor MUST NOT appear on PLAY or STUDY; the operator console MAY embed the same sketch as an operator-only Admin Watch PIXEL (authenticated sessions only, same public snapshot, no hidden-state access, never a second public map) per the operator graphics exception ([WATCH-LIGHTWEIGHT-SPECTATOR.md](docs/WATCH-LIGHTWEIGHT-SPECTATOR.md) §18.1).
