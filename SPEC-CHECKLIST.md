...
- [x] Isolated Worker/DO/SQL proof on `test.hosted-canonical.*`: shipped `workers/noema/test/isolated-settlement-proof.test.ts` (events + digest + revision bump + idempotent retry + `STALE_HEAD` + adopt/recover, never Genesis) and live `scripts/isolated-ack.mjs` ENTER 200 / Perihelion 403. Live `inspect-settlement.mjs` still needs `SUPABASE_*` (UNCONFIGURED without those secrets). This is **not** “production head missing.” [RUNTIME-READINESS-2026-08-13.md](docs/RUNTIME-READINESS-2026-08-13.md).
