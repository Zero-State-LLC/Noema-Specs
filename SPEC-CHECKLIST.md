...
- [x] Player brand implemented on hosted Worker HTML (Slices 0–9). Gate `NOEMA_PLAYER_BRAND_IMPLEMENTED`. No further brand slices unless a visual defect is filed. Scoped 2026-09-09 reopen: public `/watch` MAP WebGL / Three.js under [WATCH-REAL-TIME-MAPPING.md](docs/WATCH-REAL-TIME-MAPPING.md) §3.3; not a new visual-identity campaign.
- [x] WATCH visual direction specified: map-first diegetic composition, Phosphor-compatible glyph grammar, event-map coupling, restrained chrome, and public projection / TEXT / accessibility preservation (`docs/WATCH-VISUAL-DIRECTION.md`). Presentation only; no runtime or schema change.
...
- [x] Public `/watch` MAP WebGL allowlist (brand-lock reopen): WebGL / Three.js (or equivalent) MAY render the MAP mode stage when motion is event-born, camera targets public `room_id` / actors only, and TEXT remains complete without GL; PIXEL stays Canvas 2D until a later explicit row; anti-cosplay bans stay; MAP P0 acceptance intent is Direct-Camera / Gate D five-slot / Follow-that-teaches (product, not implement); impressiveness is a unique blend (MUD theater / living-stage MAP / civilization sports / projection honesty), not pure cinema and not a raw MUD dump ([WATCH-REAL-TIME-MAPPING.md](docs/WATCH-REAL-TIME-MAPPING.md) §3.3–§3.4, [WATCH-VISUAL-DIRECTION.md](docs/WATCH-VISUAL-DIRECTION.md) §1, [specify/watch-map-p0-gl-poi.md](specify/watch-map-p0-gl-poi.md)).
