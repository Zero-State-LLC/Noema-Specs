...
- [ ] Isolated Worker/DO/SQL proof on `test.hosted-canonical.*` only: re-runnable settle covering events → head revision → `state_digest` → idempotent retry → `STALE_HEAD` / fail-closed → adopt/recover (never Genesis). This is **not** “production head missing.” [RUNTIME-READINESS-2026-08-13.md](docs/RUNTIME-READINESS-2026-08-13.md).
